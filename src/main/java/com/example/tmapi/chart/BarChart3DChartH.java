...
public class BarChart3DChartH {
    public BarChart3DChartH(){}
...
//        // 1：创建数据集合
    ...
    public DefaultCategoryDataset createDataset(Map<String,Object> map) {
        // 标注类别
        String[] categories = new String[map.size()];
        BigDecimal[] shuzuData = new BigDecimal[map.size()];
        int i = 0;
        ...
        Vector<Serie> series = new Vector<Serie>();
        // 柱子名称：柱子所有的值集合
        series.add(new Serie("达标率:%",shuzuData ));
//        series.add(new Serie("New York", new Double[] { 83.6, 78.8, 98.5, 93.4, 106.0, 84.5, 105.0, 104.3, 91.2, 83.5, 106.6, 92.3 }));
//        series.add(new Serie("London", new Double[] { 48.9, 38.8, 39.3, 41.4, 47.0, 48.3, 59.0, 59.6, 52.4, 65.2, 59.3, 51.2 }));
//        series.add(new Serie("Berlin", new Double[] { 42.4, 33.2, 34.5, 39.7, 52.6, 75.5, 57.4, 60.4, 47.6, 39.1, 46.8, 51.1 }));
//
//     1：创建数据集合
    ...
    public ChartPanel createChart(String path, Map<String,Object> map,String title) {
        //2：创建Chart[创建不同图形]
        JFreeChart chart = ChartFactory.createBarChart(DataUtil.getNow()+"   "+title, "", "加油！必胜！", createDataset(map),PlotOrientation.HORIZONTAL,true,false, false);
        ...
        if(title.equals("总毛利达标率"))
        renderer.setSeriesPaint(0, Color.orange);
        if(title.equals("总销售达标率"))
