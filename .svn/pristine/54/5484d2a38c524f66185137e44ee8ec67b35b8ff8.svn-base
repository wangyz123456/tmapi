...
//        String[] categories = { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };
        ...
        series.add(new Serie("环比销售额",shuzuData));
...
//        Vector<Serie> series = new Vector<Serie>();
        ...
        series.add(new Serie("环比占比",shuzuData));
...
//        series.add(new Serie("Berlin", new Double[] { 42.4, 33.2, 34.5, 39.7, 52.6, 75.5, 57.4, 60.4, 47.6, 39.1, 46.8, 51.1 }));
    ...
    public void createMixedCharts(String path, ChartData chartData) {
        ...
        String[] categories = new String[chartData.getMdlstdmMap().size()];
        BigDecimal[] shuzuData = new BigDecimal[chartData.getMdlstdmMap().size()];

        int i = 0;
        for (Map.Entry<String, Object> entry : chartData.getMdlshbMap().entrySet()) {
            categories[i]=entry.getKey();
            i++;
        }

        for(int j = 0; j < categories.length; j++){
            shuzuData[j] = (BigDecimal) chartData.getMdlstdmMap().get(categories[j]);
        }



        CategoryDataset dataSetColumn = createBarDataset(chartData.getMdlstdMap(),shuzuData,categories);
        CategoryDataset dataSetLine = createLineDataset(chartData.getMdlshbMap(),categories);
...
//        Font font = new Font("宋体", Font.BOLD, 5);
        // 创建图形对象
        JFreeChart jfreeChart = ChartFactory.createBarChart(DataUtil.format(new Date(),DataUtil.FORMAT_LONOGRAM_CN)+"  门店零售环比", // 图表标题
                "", // 目录轴的显示标签
                "",// 数值轴的显示标签
                dataSetColumn, // 数据集
